...
echo '17.chacha20-ietf-poly1305'
echo '18.xchacha20-ietf-poly1305'
echo '19.salsa20'
	while :; do echo
		read -p "输入新加密方式： " um
		if [[ ! $um =~ ^[1]{0,1}[0-9]{1}$ ]]; then
			echo "输入错误! 请输入正确的数字!"
		else
			break	
		fi
...
if [[ $um == 8 ]];then
	um1="camellia-128-cfb"
fi
if [[ $um == 9 ]];then
	um1="camellia-256-cfb"
fi
if [[ $um == 10 ]];then
	um1="bf-cfb"
fi
...
if [[ $um == 17 ]];then
	um1="chacha20-ietf-poly1305"
fi
if [[ $um == 18 ]];then
	um1="xchacha20-ietf-poly1305"
fi
if [[ $um == 19 ]];then
	um1="salsa20"
fi
